...
import {
  fetchAllQuestionnairesAdmin,
  fetchQuestionnaireByIdAdmin,
...
const AdminFamilyQuestionnaires = () => {
  ...
  useEffect(() => {
    ...
    fetchQuestionnaireByIdAdmin(selectedId).then(setDetail).catch(() => setDetail(null));
  }, [selectedId]);
  ...
  return (
    <div>
      <PageHeader
        title="Questionnaires familles"
        subtitle="Réponses des familles d'accueil — à analyser pour le matching."
      ...
      <div className="admin-submissions-layout">
        <div className="admin-table-card">
          {loading ? (
            ...
            <EmptyState title="Aucun questionnaire" description="Les familles verront le formulaire dans leur espace." />
          ) : (
        ...
        {detail && (
          <aside className="admin-submission-detail fq-admin-detail">
            <div className="admin-submission-detail-head">
              <h3>{detail.profiles?.full_name || 'Famille'}</h3>
            ...
            <div className="fq-admin-scroll">
              {FAMILY_QUESTIONNAIRE_SECTIONS.map((section) => (
                <div key={section.id} className="fq-admin-section">
                  <h4>{section.title}</h4>
...
export default AdminFamilyQuestionnaires;
