...
import {
  FAMILY_QUESTION_FIELD_OVERRIDES,
  FAMILY_QUESTIONNAIRE_SECTIONS,
  QUESTIONNAIRE_META,
} from '../../../data/familyQuestionnaireSchema';
import {
  fetchMyQuestionnaire,
  saveQuestionnaireDraft,
  submitQuestionnaire,
} from '../../../services/familyQuestionnaire';
import { isSupabaseConfigured } from '../../../lib/supabase';
import './FamilyQuestionnaire.css';

const emptyAnswers = () =>
  Object.fromEntries(
    FAMILY_QUESTIONNAIRE_SECTIONS.flatMap((s) => s.questions.map((q) => [q.id, ''])),
  );

const FamilyQuestionnaire = () => {
  ...
  const section = FAMILY_QUESTIONNAIRE_SECTIONS[step];
  const totalSteps = FAMILY_QUESTIONNAIRE_SECTIONS.length;
  ...
  const getQuestionConfig = (question) => ({
    ...
    ...question,
    ...(FAMILY_QUESTION_FIELD_OVERRIDES[question.id] || {}),
  });
  ...
  const handleSaveDraft = async () => {
    ...
    setError(null);
    try {
      await saveQuestionnaireDraft(answers);
  ...
  const allComplete = FAMILY_QUESTIONNAIRE_SECTIONS.every((s) =>
  ...
  if (loading) {
    return <p style={{ padding: '2rem', color: 'var(--dash-gray-500)' }}>Chargement…</p>;
  }
  ...
  return (
    <div className="fq-page">
      ...
      <PageHeader
        title={QUESTIONNAIRE_META.title}
        subtitle={QUESTIONNAIRE_META.tagline}
      />

      <div className="fq-intro dash-card">
        {QUESTIONNAIRE_META.intro.map((p) => (
          <p key={p}>{p}</p>
        ))}
      </div>
